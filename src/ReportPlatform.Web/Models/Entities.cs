...
public sealed class Report : Entity
{
    ...
    public List<ReportFilterTemplate> FilterTemplates { get; set; } = [];
...
public sealed record ReportFilterTemplate(string Id, string Name, List<Filter> Filters);
public sealed record SaveFilterTemplateRequest(string Name, List<Filter> Filters);
public sealed record QueryRequest(List<Filter>? Filters, int Page = 1, int PageSize = 50);
public sealed record LoginRequest(string Username, string Password, string? AccountId);
public sealed record ChangePasswordRequest(string CurrentPassword, string NewPassword);
public sealed record ReportFieldsPreviewRequest(string ConnectionId, string Sql);
