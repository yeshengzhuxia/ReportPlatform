...
3. **报表**：维护 SELECT、组织字段、显示字段、中文名称、分类、快捷菜单和自定义图标。组织字段留空时不按账套过滤，会显示所有组织数据。
...
- 打开报表后，标题、模板选择、过滤、查询与导出集中在紧凑工具栏，查询列表使用剩余窗口空间。
- 点击“过滤条件”在弹窗中编辑；“应用条件”后点击“查询数据”刷新结果，取消不会改变已应用条件。
- 弹窗中填写名称并点击“保存为我的模板”，即可在下次从模板下拉框选择。个人模板按用户、账套和报表保存到平台数据库。
- 有报表配置权限的用户可进入“设置 → 过滤模板与默认条件”，新增报表预设，选择默认模板并保存配置。默认模板在打开报表时带入，不自动执行查询。
- “恢复默认”带回报表默认条件；需要查询全部数据时，可在过滤弹窗中清空并应用条件。
- 预设不授予额外权限。配置了组织字段的报表仍由登录账套限定范围；组织字段留空的报表按配置显示全部组织。报表字段变化后，失效的个人模板会标记并禁止直接应用，需编辑条件或删除。
SQL 示例：
...
组织字段填写 `OrgId`，显示字段例如 `OrderNo → 单据编号`、`Amount → 金额`。填写组织字段时，该字段必须在 SQL 返回列中且可以不显示；SQL 外层强制 `OrgId = @org`，组织 ID 来自登录账套，浏览器不能覆盖。组织字段留空时不会添加组织条件，将显示全部组织的数据。所有页面筛选值仍使用参数。
...
- 查询支持分页，每页可选 20–1000 条；CSV 采用流式导出，不设导出行数上限。查询命令超时 30 秒，CSV 防护公式注入。
...
- 默认 HTTP 集成测试使用替身查询执行器；设置 REPORT_PLATFORM_SQL_CHECK_ROOT 为源码根目录可显式运行 SQL Server 只读联调测试，未设置时跳过。
...
- .NET 测试覆盖密码兼容、SQL 参数化、CSV 公式防护、登录和账套授权、按钮权限、分页导出、菜单配置、模板持久化与归属隔离、审计与会话撤销。
